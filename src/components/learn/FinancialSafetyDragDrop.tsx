...
interface FinancialSafetyDragDropProps {
  title: string;
  instruction: string;
  items: DragDropItem[];
...
const FinancialSafetyDragDrop: React.FC<FinancialSafetyDragDropProps> = ({
  title,
  instruction,
  items,
  categories,
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {unassignedItems.map(item => (
              <div
                key={item.id}
                className="px-3 py-2 bg-purple-100 border border-purple-300 rounded-lg cursor-move text-sm hover:bg-purple-200 transition-colors"
                draggable
        ...
        {Object.keys(assignments).length === items.length && !showResults && (
          <Button onClick={checkAnswers} className="w-full bg-purple-500 hover:bg-purple-600">
            Check My Answers
          </Button>
        )}
...
export default FinancialSafetyDragDrop;
