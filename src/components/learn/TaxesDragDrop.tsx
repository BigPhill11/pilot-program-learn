...
import { DragDropActivity } from '@/data/taxes-journey-data';

interface TaxesDragDropProps {
  activity: DragDropActivity;
  onComplete: (isCorrect: boolean) => void;
}

const TaxesDragDrop: React.FC<TaxesDragDropProps> = ({ activity, onComplete }) => {
          ...
          <div className="flex flex-wrap gap-2">
            {getUnplacedItems().map(item => (
              <div
                ...
                className="bg-primary/10 border border-primary/30 rounded-lg px-3 py-2 cursor-move text-sm hover:bg-primary/20 transition-colors"
              >
                {item?.text}
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {activity.categories.map(category => {
          ...
          return (
            <Card
              key={category.id}
              className="border-2 border-dashed border-gray-300 hover:border-primary/50 transition-colors"
            >
              <CardHeader>
