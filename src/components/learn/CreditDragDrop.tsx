...
import { CreditDragDropActivity } from '@/data/credit-journey-data';

interface CreditDragDropProps {
  activity: CreditDragDropActivity;
  onComplete: (isCorrect: boolean) => void;
}

const CreditDragDrop: React.FC<CreditDragDropProps> = ({ activity, onComplete }) => {
          ...
          <div className="flex flex-wrap gap-2">
            {getUnplacedItems().map(item => (
              <div
                ...
                className="bg-green-500/10 border border-green-500/30 rounded-lg px-3 py-2 cursor-move text-sm hover:bg-green-500/20 transition-colors"
              >
                {item?.text}
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {activity.categories.map(category => {
          ...
          return (
            <Card
              key={category.id}
              className="border-2 border-dashed border-gray-300 hover:border-green-500/50 transition-colors"
            >
              <CardHeader>
