...
import { FuturePlanningDragDropActivity } from '@/data/future-planning-journey-data';

interface FuturePlanningDragDropProps {
  activity: FuturePlanningDragDropActivity;
  onComplete: (isCorrect: boolean) => void;
}

const FuturePlanningDragDrop: React.FC<FuturePlanningDragDropProps> = ({
  activity,
  onComplete
}) => {
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-indigo-600 mb-2">{activity.title}</h3>
        ...
        <div className="flex flex-wrap gap-2">
          {unassignedItems.map(item => (
            <div
              ...
              className="px-3 py-2 bg-indigo-100 border border-indigo-300 rounded-lg cursor-move hover:bg-indigo-200 transition-colors text-sm"
            >
              {item.content}
            </div>
          ))}
          ...
          return (
            ...
            >
              <CardHeader className="pb-3">
                <CardTitle className="text-base text-indigo-600">{category.title}</CardTitle>
              ...
              <CardContent className="pt-0">
                <div className="min-h-[100px] space-y-2">
                  {assignedItems.map(item => (
                    <div
                      key={item?.id}
                      className="px-2 py-1 bg-indigo-50 border border-indigo-200 rounded text-sm"
                    >
                      {item?.content}
                      {showResults && (
                        <span className="ml-2">
      ...
      {unassignedItems.length === 0 && !showResults && (
        <div className="text-center">
          <Button onClick={checkAnswers} className="bg-indigo-500 hover:bg-indigo-600">
            Check My Answers
          </Button>
        </div>
      )}
...
export default FuturePlanningDragDrop;
